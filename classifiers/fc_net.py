...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    #
    # dhidden, dW2, db2 = affine_backward(dsoftmax, cache2)
    # dx, dW1, db1 = affine_relu_backward(dhidden, cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    if self.num_layers == 1:
      ...
      i += 1
      self.params['W%d'%i] = weight_scale * np.random.randn(hidden_dims[i-2], num_classes)
  ...
  def loss(self, X, y=None):
    ...
    if L == 1:
      ...
      # print(type(self.params['W1']))
      out, cache1 = affine_relu_forward(X, self.params['W1'], self.params['b1'])
      if self.use_dropout:
        out, cache1_dropout = dropout_forward(out, self.dropout_param)
      ...
      i += 1
      scores, cache[i] = affine_forward(x, self.params['W%d'%i], self.params['b%d'%i])
    ...
    if L == 1:
      dx, dW1, db1 = affine_backward(dscores, cache1)
      ...
      if self.use_dropout:
        dhidden = dropout_backward(dhidden, cache1_dropout)
      dx, dW1, db1 = affine_relu_backward(dhidden, cache1)
      dW1 += self.reg * self.params['W1']
      grads = {'W1':dW1, 'b1':db1, 'W2':dW2, 'b2':db2}

    else:
      dhidden, grads['W%i'%L], grads['b%i'%L] = affine_backward(dscores, cache[L])
      grads['W%i'%L] += self.reg * self.params['W%i'%L]
      for i in xrange(L-1, 0, -1):
        if self.use_dropout:
          dhidden = dropout_backward(dhidden, cache_dropout[i])
        dhidden, grads['W%i'%i], grads['b%i'%i] = affine_relu_backward(dhidden, cache[i])
